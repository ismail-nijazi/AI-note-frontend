...
const normalizeErrorMessage = (
  ...
  if (
    ...
    const recordBody = errorBody as Record<string, unknown>;

    if (typeof recordBody.message === "string") {
      return recordBody.message || fallbackMessage;
    }
    if (Array.isArray(recordBody.message)) {
      return normalizeErrorMessage(recordBody.message, fallbackMessage);
    }
    if (typeof recordBody.error === "string") {
      return recordBody.error || fallbackMessage;
    }
    if (Array.isArray(recordBody.error) && recordBody.error.length > 0) {
      return normalizeErrorMessage(recordBody.error, fallbackMessage);
    }
    const issueMessage = (
    ...
    if (typeof issueMessage === "string") {
      return issueMessage.trim() || fallbackMessage;
    }
  }
...
const parseAuthResponse = async (
  ...
  if (!response.ok) {
    ...
    if (status === 404) {
      ...
      message = normalizeErrorMessage(errorBody, fallbackMessage);
    }
...
const loginWithEmail = async (payload: {
  ...
  try {
    ...
    return await parseAuthResponse(response, "Invalid email or password.");
  } catch (error) {
...
const loginWithGoogle = async (): Promise<AuthenticatedUser> => {
  throw new Error(
