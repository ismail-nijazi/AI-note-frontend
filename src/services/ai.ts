...
export class AIService {
  async *generate(options: AIGenerateOptions): AsyncIterable<AIStreamChunk> {
    const { messages, context, model, maxTokens, temperature, stream } =
      options;

    try {
      console.log("AI Service: Starting generation with options:", options);
    ...
    try {
      while (true) {
        ...
        for (const line of lines) {
          ...
          const chunk = this.parseDataLine(line, currentEvent);
          if (chunk) {
            yield chunk;
          }
        }
      ...
      if (buffer.trim()) {
        ...
        for (const line of lines) {
          ...
          const chunk = this.parseDataLine(line, finalEvent);
          if (chunk) {
            yield chunk;
          }
        }
  ...
  private parseDataLine(
    ...
    try {
      ...
      console.log("AI Service: Parsed SSE data:", {
        event: currentEvent,
        data,
      });
      ...
      if (this.isTextEvent(currentEvent, data)) {
        if (data.fullResponse) {
          console.log(
            ...
            data.fullResponse.substring(0, 50) + "...",
          );
      ...
      if (data.type === SSE_EVENTS.FUNCTION_RESULT) {
  ...
  private throwSSEError(data: SSEData): never {
    console.error("AI Service: Received error:", data.error);

    const errorMessage = data.error?.message || "AI generation failed";
    const errorCode = data.error?.code || "AI_GENERATION_FAILED";
  ...
  private async *generateMockResponse(
    ...
    const response = this.getMockResponse(userInput, options.context);
  ...
  private getMockResponse(input: string, context?: AIContext): string {
    ...
    for (const [command, response] of Object.entries(quickActions)) {
    ...
    // Default response
    const contextPrefix = context ? "Based on your note context, " : "";
