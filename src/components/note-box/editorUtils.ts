...
const URL_PROTOCOL_REGEX = /^[a-zA-Z][\w+.-]*:\/\//;
...
export const createNoteBoxEditor = (): NoteEditor =>
  withInlines(withHistory(withReact(createEditor()))) as NoteEditor;

export const cloneContent = (content: Descendant[]): Descendant[] =>
  JSON.parse(JSON.stringify(Array.isArray(content) ? content : []));
...
export const wrapLink = (editor: Editor, url: string, displayText?: string) => {
...
export const ensureLinkForSelection = (editor: Editor, url: string) => {
  wrapLink(editor, url);
};
