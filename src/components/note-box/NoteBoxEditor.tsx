...
import type { CustomElement, CustomText, NoteEditor } from "./types";
...
export const NoteBoxEditor: React.FC<NoteBoxEditorProps> = ({
  editor,
  slateKey,
  value,
  isEditing,
  ...
  const renderElement = useCallback(
    (props: RenderElementProps) => {
      ...
      switch (element.type) {
        ...
        case "numbered-list":
          return (
            <ol className="list-decimal pl-6" {...props.attributes}>
              {props.children}
            </ol>
          );
        case "list-item":
  ...
  const renderLeaf = useCallback((props: RenderLeafProps) => {
    ...
    const style =
      leaf.fontSize && leaf.fontSize !== 14
        ? {
  ...
  return (
    ...
    >
      <Editable
        ...
        onBlur={onBlur}
        onMouseDown={(event) => {
          const linkTarget = (event.target as HTMLElement).closest("a");
          if (linkTarget && !isEditing) {
            event.stopPropagation();
            return;
          }
          if (isEditing) {
