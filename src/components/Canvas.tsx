...
export const Canvas: React.FC<CanvasProps> = ({ onToolbarCallbacksChange }) => {
  ...
  const { getCurrentNote, updateNote, workspace, leftSidebarOpen } =
    useWorkspaceStore();
  ...
  const saveCurrentNoteNow = useCallback(async () => {
    ...
    if (
      ...
      try {
        ...
        await syncNoteToBackend(workspace.active.noteId);
          ...
          <div className="text-center text-muted-foreground">
            ...
            {!currentNote && (
              <p className="text-sm mt-4 text-primary">
                Create a collection and note from the left sidebar to get
                started
              </p>
            )}
          </div>
