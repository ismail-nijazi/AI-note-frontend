...
import { useBoardStore, NoteBox as NoteBoxType } from '@/state/useBoardStore';
...
const withInlines = (editor: BaseEditor & ReactEditor) => {
  ...
  editor.isInline = element => {
    return (element as CustomElement).type === 'link' ? true : isInline(element);
  };
...
export const NoteBox: React.FC<NoteBoxProps> = ({
  ...
  const { updateNoteBox, bringToFront, deleteNoteBox, duplicateNoteBox, saveToHistory, canvasTransform, editingBoxId, setEditingBox } = useBoardStore();
  ...
  const formatCallbacks = {
    ...
    onFontSize: (size: string) => {
      ...
      if (fontSize !== 14) { // 14 is default, don't store it
    ...
    getCurrentFontSize: () => {
      const marks = Editor.marks(editor);
      if (!marks || !marks.fontSize) return '14';
  ...
  const renderElement = useCallback((props: any) => {
    ...
    switch (element.type) {
      case 'block-quote':
        return <blockquote className="border-l-4 border-border pl-4 italic text-muted-foreground" {...props.attributes}>{props.children}</blockquote>;
      case 'bulleted-list':
      ...
      case 'heading-one':
        return <h1 className="text-2xl font-bold mb-2" {...props.attributes}>{props.children}</h1>;
      case 'heading-two':
        return <h2 className="text-xl font-semibold mb-2" {...props.attributes}>{props.children}</h2>;
      case 'code-block':
        return <pre className="bg-muted p-2 rounded font-mono text-sm overflow-x-auto" {...props.attributes}><code>{props.children}</code></pre>;
      case 'link':
        return <a href={element.url} className="text-primary underline" {...props.attributes}>{props.children}</a>;
      case 'image':
        return (
  ...
  const renderLeaf = useCallback((props: any) => {
    ...
    // Handle font sizes
    let style = {};
    if (leaf.fontSize && leaf.fontSize !== 14) {
  ...
  const handleChange = (value: Descendant[]) => {
    updateNoteBox(noteBox.id, { content: value });
  };
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    ...
    const actions = [
      { label: 'Duplicate', action: () => duplicateNoteBox(noteBox.id) },
      { label: 'Bring to Front', action: () => bringToFront(noteBox.id) },
      { label: 'Send to Back', action: () => { /* sendToBack not implemented in store */ } },
      { label: 'Delete', action: () => {
        const hasContent = noteBox.content.length > 1 || 
          (noteBox.content[0] as any)?.children?.[0]?.text !== 'Start typing...';
        if (hasContent) {
    ...
    actions.forEach(action => {
      ...
      button.className = 'block w-full text-left px-2 py-1 hover:bg-accent rounded text-sm';
  ...
  return (
    <Rnd
      size={{ width: noteBox.width, height: noteBox.height }}
      position={{ x: noteBox.x, y: noteBox.y }}
      onDrag={(e, d) => {
        // No need to adjust coordinates - react-rnd already handles the scaling
        updateNoteBox(noteBox.id, { x: d.x, y: d.y });
      }}
      onDragStop={(e, d) => {
        updateNoteBox(noteBox.id, { x: d.x, y: d.y });
        saveToHistory();
    ...
    >
      <div 
        className={`h-full p-3 overflow-y-auto no-scrollbar ${
          !isEditing ? 'drag-handle hover:bg-accent/10 transition-colors' : ''
        }`}
        style={{
      ...
      >
        ...
        >
          <Editable
            ...
            className={`outline-none text-sm leading-relaxed ${isEditing ? 'cursor-text' : 'cursor-move'}`}
            style={{
