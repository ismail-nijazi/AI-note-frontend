...
interface BoardActions {
  ...
  insertContentToBox: (id: string, text: string) => void;
  replaceBoxContent: (id: string, text: string) => void;
  appendTextToBox: (id: string, text: string) => void;
  bringToFront: (id: string) => void;
  sendToBack: (id: string) => void;
  duplicateNoteBox: (id: string) => void;
  updateCanvasTransform: (
    transform: Partial<BoardState["canvasTransform"]>,
  ) => void;
  undo: () => void;
  redo: () => void;
  saveToHistory: () => void;
...
export const useBoardStore = create<BoardState & BoardActions>((set, get) => ({
  ...
  insertContentToBox: (id: string, text: string) => {
    const box = get().noteBoxes.find((b) => b.id === id);
    if (!box) return;
    ...
    set(
      produce((state: BoardState) => {
        const box = state.noteBoxes.find((b) => b.id === id);
        if (box) {
          // Append new paragraph to existing content
          box.content = [...box.content, newParagraph];
        }
      }),
    );
  ...
  replaceBoxContent: (id: string, text: string) => {
  ...
  appendTextToBox: (id: string, text: string) => {
    const box = get().noteBoxes.find((b) => b.id === id);
    ...
    set(
      produce((state: BoardState) => {
        const box = state.noteBoxes.find((b) => b.id === id);
        if (box && box.content.length > 0) {
          const lastElement = box.content[box.content.length - 1];
          if (lastElement && "children" in lastElement) {
            const lastTextNode =
              lastElement.children[lastElement.children.length - 1];
            if (lastTextNode && "text" in lastTextNode) {
      ...
      if (saved) {
        ...
        set(
          produce((state: BoardState) => {
            ...
            state.canvasTransform = parsedState.canvasTransform || {
              x: 0,
              y: 0,
              scale: 1,
            };
  ...
  syncNoteToBackend: async (noteId: string) => {
    try {
      ...
      const { collectionId } = ws.workspace.active;
      const note = collectionId
        ? ws.workspace.collections[collectionId]?.notes[noteId]
        : undefined;

      const currentVersion = note?.version || 1;

      const attemptUpdate = async (version: number) => {
        // Prefer WS update
        ...
        setTimeout(async () => {
          try {
            const res = await apiService.updateNote(noteId, {
              content: noteBoxes,
              version,
            });
            ...
            useWorkspaceStore.setState(
              produce((state: { workspace: Workspace }) => {
                if (collectionId) {
                  ...
                  if (n) n.version = updated.version || n.version + 1;
                }
              }),
            );
          } catch (error) {
            console.error("Failed to update note via HTTP fallback:", error);
          }
        }, 500);
      };

      try {
        await attemptUpdate(currentVersion);
      } catch (e: unknown) {
        // If 409, fetch current version and retry once
        if (e instanceof Error && /status:\s*409/.test(e.message)) {
          const latestRes = await apiService.getNote(noteId);
          ...
          useWorkspaceStore.setState(
            produce((state: { workspace: Workspace }) => {
              if (collectionId) {
                const n =
                  state.workspace.collections[collectionId]?.notes[noteId];
                if (n) n.version = latest.version || n.version;
              }
            }),
          );
          await attemptUpdate(latest.version || currentVersion);
        } else {
          throw e;
        }
      }
...
socketService.on((msg) => {
  ...
  const boxesSource = Array.isArray(content?.boxes) ? content.boxes : content;
