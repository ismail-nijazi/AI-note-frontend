...
import { socketService, type InboundMessage } from "@/services/socket";
...
interface WorkspaceActions {
  ...
  updateCollection: (
    id: string,
    updates: Partial<Omit<Collection, "id" | "notes">>,
  ) => Promise<void>;
  deleteCollection: (id: string) => Promise<void>;
  reorderCollections: (newOrder: string[]) => void;

  // Note management
  createNote: (collectionId: string, title: string) => Promise<string>;
  updateNote: (
    collectionId: string,
    noteId: string,
    updates: Partial<Omit<Note, "id">>,
  ) => Promise<void>;
  deleteNote: (collectionId: string, noteId: string) => Promise<void>;
  reorderNotes: (collectionId: string, newOrder: string[]) => void;
  duplicateNote: (collectionId: string, noteId: string) => Promise<string>;
...
export const useWorkspaceStore = create<WorkspaceState & WorkspaceActions>(
  (set, get) => ({
        ...
        set(
          produce((state: WorkspaceState) => {
            ...
            state.workspace.collectionOrder =
              state.workspace.collectionOrder.filter((cId) => cId !== id);
    ...
    createNote: async (collectionId: string, title: string) => {
      try {
        ...
        set(
          produce((state: WorkspaceState) => {
            ...
            if (collection) {
              ...
              collection.noteOrder = collection.noteOrder.filter(
                (nId) => nId !== noteId,
              );
    ...
    duplicateNote: async (collectionId: string, noteId: string) => {
      try {
        const response = await apiService.duplicateNote(noteId, {
          title: `${
            get().workspace.collections[collectionId]?.notes[noteId]?.title
          } (Copy)`,
          collectionId,
        });
        ...
        if (saved) {
          ...
          set(
            produce((state: WorkspaceState) => {
              ...
              state.workspace.collections = parsedState.workspace.collections;
              state.workspace.collectionOrder =
    ...
    loadFromBackend: async () => {
      try {
        ...
        const collections = collectionsJson.data || collectionsJson || [];

        // Load notes for each collection
        const collectionsWithNotes: Record<string, Collection> = {};
        ...
        for (const collection of collections) {
          ...
          const notes = notesJson.data || notesJson || [];
          ...
          notes.forEach((note: BackendNote) => {
            notesMap[note.id] = {
              id: note.id,
              title: note.title,
              boxes: note.content || [],
          ...
          collectionsWithNotes[collection.id] = {
            id: collection.id,
            title: collection.name,
            noteOrder: notes.map((note: Note) => note.id),
            notes: notesMap,
          };
        }
        ...
        set(
          produce((state: WorkspaceState) => {
            state.workspace.collections = collectionsWithNotes;
            state.workspace.collectionOrder = collections.map(
              (c: Collection) => c.id,
            );
          }),
        );
...
socketService.on((msg: InboundMessage) => {
  if (msg.type === "note:updated") {
    ...
    const { collectionId } = state.workspace.active;
    if (!collectionId) return;
    useWorkspaceStore.setState((prev) => {
      const next = JSON.parse(JSON.stringify(prev));
      const n = next.workspace.collections[collectionId]?.notes[noteId];
      if (n) {
